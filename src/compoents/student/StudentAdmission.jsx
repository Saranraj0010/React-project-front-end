...
const StudentAdmission = () => {
    ...
    const input = "pl-4 h-10 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition";
    ...
    const AddFrom = async (e) => {
        try {
            ...
            setPayment({
                ...
                fees: selectedFees?.fees || 0,
                downPayment: "",
                balance: selectedFees?.fees || 0
            });
    ...
    const downPayment = async (e) => {
        ...
        try {
            ...
            if (!Validation()) return
            console.log(payment)
            const add = await axios.post(`${API}addPayment`, payment)
            setFees(false)
        }
        catch (err) {
            console.log(err)
                ...
                <form className={`w-full p-5 border rounded-lg flex flex-col gap-4`} onSubmit={(e) => AddFrom(e)}>
                    ...
                    <div className="md:grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                        <div className="flex flex-col gap-2">
                            ...
                            <input name="roleNo" ref={(el) => (inputRef.current["roleNo"] = el)} value={student.roleNo} placeholder="Role Number" className={input} onChange={(e) => { setStudent({ ...student, roleNo: e.target.value }), setError({ ...error, roleNo: "" }) }} />
                            {error.roleNo && (
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input name="lastName" ref={(el) => (inputRef.current["lastName"] = el)} value={student.lastName} placeholder="Last Name" className={input} onChange={(e) => { setStudent({ ...student, lastName: e.target.value }), setError({ ...error, lastName: "" }) }} />
                            {error.lastName && (
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setPayment({
                                            ...payment,
                                            downPayment: value,
                                            balance: (payment.fees || 0) - value
                                        }), setError({ ...error, downPayment: "" })
                                    }}
                                />
                                {error.downPayment && (
